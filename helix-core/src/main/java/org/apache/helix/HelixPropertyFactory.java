      ...
      if (Boolean.getBoolean(SystemPropertyKeys.MULTI_ZK_ENABLED) || zkAddress == null) {
        ...
        try {
          if (realmAwareZkConnectionConfig == null) {
            // or we can use user provided config first and fall back if it can  not connect?
            //RealmAwareZkClient.RealmAwareZkConnectionConfig connectionConfig
            realmAwareZkConnectionConfig =
                new RealmAwareZkClient.RealmAwareZkConnectionConfig.Builder().setRealmMode(RealmAwareZkClient.RealmMode.SINGLE_REALM)
